...
pub(crate) fn query_provider_state(
    ...
    let executor = open_api_executor.ok_or_else(|| {
        RtcContractError::Unavailable(
            "tencent active query requires a configured OpenAPI executor".to_string(),
        )
    })?;
...
fn parse_provider_response(
    ...
    let request_id = parsed.as_ref().ok().and_then(tencent_request_id);
    if !(200..300).contains(&response.status_code) {
        return Err(RtcContractError::Unavailable(format!(
            "tencent active query returned HTTP {}{}",
    ...
    let body = parsed.map_err(|error| {
        RtcContractError::Unavailable(format!(
            "failed to parse tencent active query response JSON: {error}"
        ))
    })?;
    if let Some(error) = tencent_error(&body) {
        return Err(RtcContractError::Unavailable(format!(
            "tencent active query failed with provider error {}{}{}",
...
fn normalized_snapshot(
    ...
    normalized.insert("provider".into(), json!("tencent"));
    normalized.insert("action".into(), json!(action));
    normalized.insert("status".into(), json!("synced"));
    insert_optional_string(&mut normalized, "requestId", tencent_request_id(body));
...
fn tencent_error(body: &JsonValue) -> Option<ProviderErrorSummary> {
    let error = get_path(body, &["Response", "Error"]).or_else(|| body.get("Error"))?;
    Some(ProviderErrorSummary {
        code: first_string_from_paths(error, &[&["Code"][..], &["code"][..], &[][..]]),
        message: first_string_from_paths(error, &[&["Message"][..], &["message"][..]]),
    })
}

fn tencent_request_id(body: &JsonValue) -> Option<String> {
    first_string_from_paths(
        body,
        &[
            &["Response", "RequestId"][..],
