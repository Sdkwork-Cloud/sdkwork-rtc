...
test("sdkwork-rtc integrates sdkwork-database framework for persistence bootstrap", () => {
  ...
  assert.match(apiBootstrap, /connect_rtc_persistence_bootstrap_from_env/u, "standalone-gateway must bootstrap persistence through repository database module");
...
test("sdkwork-rtc ships production deployment manifests and reconcile binary", () => {
  for (const filePath of [
    ...
    "deployments/kubernetes/cloud/namespace.yaml",
    "deployments/kubernetes/cloud/rtc-standalone-gateway/deployment.yaml",
    "deployments/kubernetes/cloud/rtc-standalone-gateway/service.yaml",
    "deployments/kubernetes/cloud/rtc-reconcile/cronjob.yaml",
