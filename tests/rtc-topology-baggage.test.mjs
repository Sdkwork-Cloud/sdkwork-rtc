...
const skipPathFragments = [
  ...
  '/generated/',
  '/.dart_tool/',
  'rtc-topology-baggage.test.mjs',
  'rtc-topology-contract.test.mjs',
...
const bannedPatterns = [
  { id: 'topology v1 env key', pattern: /SDKWORK_RTC_TOPOLOGY/u },
  { id: 'client topology v1 env key', pattern: /VITE_SDKWORK_RTC_TOPOLOGY/u },
  { id: 'topology CLI flag', pattern: /--topology\b/u },
  { id: 'legacy bind env', pattern: /SDKWORK_RTC_API_BIND/u },
  { id: 'legacy client app api env', pattern: /VITE_RTC_API_BASE_URL/u },
  { id: 'legacy client backend api env', pattern: /VITE_RTC_BACKEND_API_BASE_URL/u },
  { id: 'legacy rtc dev port', pattern: /127\.0\.0\.1:18080/u },
];
...
assert.ok(
  fs.existsSync(path.join(repoRoot, 'scripts/lib/rtc-topology.mjs')),
  'rtc topology adapter required',
);
...
assert.match(
  ...
  'rtc-runtime.env.example must document topology bind env',
);
assert.doesNotMatch(
  runtimeExample,
  /SDKWORK_RTC_API_BIND/u,
  'rtc-runtime.env.example must not use retired SDKWORK_RTC_API_BIND',
);

console.log('[rtc-topology-baggage] ok');
