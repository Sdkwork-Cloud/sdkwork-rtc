use sdkwork_utils_rust::{
    offset_window_page_info, CursorListPageParams, SdkWorkPageData, SdkWorkResultCode,
};
...
pub fn rtc_list_page_to_sdkwork_page_data<T>(
    ...
    SdkWorkPageData {
        items: page.items,
        page_info: offset_window_page_info(Some(page_size), page.next_cursor, has_more),
    }
}
...
pub fn resolved_list_page_size(params: &RtcListWindowParams) -> usize {
    ...
    CursorListPageParams::resolve(Some(page_size), params.cursor.as_deref())
...
mod tests {
    ...
    #[test]
    fn list_page_builds_offset_page_info_for_numeric_cursors() {
        ...
        assert_eq!(PageMode::Offset, data.page_info.mode);
