...
/** Rtc rooms list. */
  async list(params?: RtcRoomsRtcRoomsListParams, requestOptions?: ApiRequestOptions): Promise<{ items: RtcRoom[]; pageInfo: { mode: 'cursor'; nextCursor?: string | null; hasMore: boolean; }; }> {
    ...
    return this.client.request<{ items: RtcRoom[]; pageInfo: { mode: 'cursor'; nextCursor?: string | null; hasMore: boolean; }; }>(appendQueryString(appApiPath(`/rtc/rooms`), query), { ...(requestOptions?.signal !== undefined ? { signal: requestOptions.signal } : {}), ...(requestOptions?.timeout !== undefined ? { timeout: requestOptions.timeout } : {}), method: 'GET' as any, sdkworkUnwrapKind: 'page' });
  }

/** Rtc rooms retrieve. */
  async retrieve(roomId: string, requestOptions?: ApiRequestOptions): Promise<RtcRoom> {
    return this.client.request<RtcRoom>(appApiPath(`/rtc/rooms/${serializePathParameter(roomId, { name: 'roomId', style: 'simple', explode: false })}`), { ...(requestOptions?.signal !== undefined ? { signal: requestOptions.signal } : {}), ...(requestOptions?.timeout !== undefined ? { timeout: requestOptions.timeout } : {}), method: 'GET' as any, sdkworkUnwrapKind: 'item' });
  }
}
...
export function createRtcRoomsApi(client: HttpClient): RtcRoomsApi {
