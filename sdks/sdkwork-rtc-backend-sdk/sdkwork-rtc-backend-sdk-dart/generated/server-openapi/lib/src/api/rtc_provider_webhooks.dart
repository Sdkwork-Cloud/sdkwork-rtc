...
class RtcProviderWebhooksApi {
  ...
  RtcProviderWebhooksApi(this._client);

  /// Rtc provider Webhooks events receive.
  Future<RtcProviderWebhookEventResponse?> eventsReceive(String provider, RtcProviderWebhookReceiveRequest body, [String? idempotencyKey]) async {
    ...
    final response = await _client.request('POST', ApiPaths.backendPath('/rtc/provider_webhooks/${serializePathParameter(provider, const PathParameterSpec('provider', 'simple', false))}/events'), body: payload, headers: requestHeaders, contentType: 'application/json', skipAuth: true);
    return (() {
      final map = sdkworkResponseAsMap(response);
      return map == null ? null : RtcProviderWebhookEventResponse.fromJson(map);
    })();
  }

  /// Rtc provider Webhooks events list.
  Future<RtcProviderWebhookEventListResponse?> eventsList([int? page, int? pageSize, String? cursor, String? sort, String? q]) async {
    ...
    final response = await _client.get(ApiPaths.appendQueryString(ApiPaths.backendPath('/rtc/provider_webhooks/events'), query));
    return (() {
      final map = sdkworkResponseAsMap(response);
      return map == null ? null : RtcProviderWebhookEventListResponse.fromJson(map);
    })();
  }
}
