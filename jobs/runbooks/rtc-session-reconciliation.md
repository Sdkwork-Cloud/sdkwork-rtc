...
- Soft-delete aged `ready`/`failed` recording artifacts per `specs/recording-policy/platform-default.json`.
...
Recording artifact lifecycle policy authority: `specs/recording-policy/platform-default.json` (override with `SDKWORK_RTC_RECORDING_POLICY_PATH`).
